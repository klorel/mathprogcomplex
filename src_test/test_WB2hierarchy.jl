...
function main()
    ...
    for (v2max, vals) in sols
        ...
        for d=2:2:2*dmax  # Complex d definition, twice real one...
            ...
            logpath = joinpath("Mosek_runs", "testWB2extensive", "WB2_v2max_$(v2max)_d_$(d)_noeq")
    ...
    for (v2max, vals) in sols
        ...
        for d=2:2:2*dmax  # Complex d definition, twice real one...
            ...
            logpath = joinpath("Mosek_runs", "testWB2extensive", "WB2_v2max_$(v2max)_d_$(d)_eq")
            ispath(logpath) && rm(logpath, recursive=true); mkpath(logpath)
            println("Saving file at $logpath")
    ...
    logfile = joinpath(pwd(), "test_WB2.txt")
