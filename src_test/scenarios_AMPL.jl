...
include(joinpath(ROOT,"..","dev", "parse_knitro_logs.jl"))
using MAT
...
function solve_GOC_via_AMPL(data_path, folder, scenario)
    ...
    knitro_log_path = joinpath(amplexportpath, outlog)
...
for (scenario, data) in results
    ...
    write(f, "$(scenario);$(solve_result_1);$(solve_result_2);$(solve_result_3);$(feas);$(ctr1);$(min_slack);$(ctr2);$(opterror1);$(opterror2);$(opterror3)\n")
    write(f2, "$(scenario)")
...
nb_iters1 = [ parse(Int64, data[9][1]) for (scenario, data) in results]
nb_iters2 = [ parse(Int64, data[9][2]) for (scenario, data) in results]
nb_iters3 = [ parse(Int64, data[9][3]) for (scenario, data) in results]
