...
function get_JuMP_cartesian_model(problem_poly::Problem, mysolver)
    ...
    variables_jump = OrderedDict{String, JuMP.Variable}()
    ...
    ctr_jump = OrderedDict{String,JuMP.ConstraintRef}()
...
function poly_to_NLexpression(m::JuMP.Model, variables_jump::OrderedDict{String, JuMP.Variable},polynome::Polynomial)
    s = 0
...
function get_JuMP_polar_model(pb::Problem, mysolver)
    m = Model(solver = mysolver)
    jump_vars = OrderedDict{String, JuMP.Variable}()
    ...
    for (ctr, modeler_ctr) in pb.constraints
        ...
        lb = modeler_ctr.lb
        ub = modeler_ctr.ub
        ps = OrderedDict{Exponent, Tuple{Real, Real}}(exp=>(real(coeff), imag(coeff)) for (exp, coeff) in p)
    ...
    ps = OrderedDict{Exponent, Tuple{Real, Real}}(exp=>(real(coeff), imag(coeff)) for (exp, coeff) in polynome_obj)
