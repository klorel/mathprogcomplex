...
function create_vars!(element::T, bus::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}, scenario::String) where T <: IIDMLoad
...
## 2. Power balance
function Snodal(element::T, busid::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}) where T <: IIDMLoad
  cstrname = "LOAD"
...
function constraint(element::T, busid::String, elemid::String, elem_formulation::Symbol, bus_vars::SortedDict{String, Variable}) where T <: IIDMLoad
  if elem_formulation == :NewVar
    cstrname = "Cstr_$(busid)_$(elemid)"
    Sload = element.load
    p = Polynomial(bus_vars[elemid])
    return SortedDict{String, Constraint}(cstrname=> p == Sload)
  end
