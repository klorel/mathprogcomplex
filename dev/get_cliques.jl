...
function convert_mipb_to_pb(pb::Problem)
    ...
    pb2 = Problem()
    pb2.constraints = pb.constraints
    pb2.objective = pb.objective
    for var in variables
        if var[2] == Bool
            variable = Variable(var[1], Real)
            add_variable!(pb2, Variable(var[1], Real))
            add_constraint!(pb2, "Bin_$(var[1])", variable^2 - variable == 0)
        else
            add_variable!(pb2, Variable(var[1], var[2]))
        end
    end
    return pb2
end
...
#
# pb2 = convert_mipb_to_pb(PB_poly_real)
#
# print(pb2)
#
# print(pb2.variables)
